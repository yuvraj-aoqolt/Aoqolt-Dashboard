...
import { FiArrowRight, FiClock, FiCheck } from 'react-icons/fi'
...
export default function ServicesPage() {
  ...
  const handleSelectService = (service) => {
    if (!isAuthenticated) {
      navigate('/login', { state: { from: `/booking/${service.id}`, serviceId: service.id } })
    } else {
      ...
      <div className="relative z-10 p-8">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-white/5">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center gap-2 bg-gradient-to-r from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 text-white text-sm font-semibold px-5 py-2.5 rounded-xl shadow-md shadow-red-900/40 transition-all duration-300"
          >
            Book Now
